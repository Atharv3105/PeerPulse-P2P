...
router.get('/', async (req, res) => {
  try {
    ...
    const formatted = loans.map(l => ({
      ...
      tenure: l.tenure,
      purpose: l.purpose,
      sector: l.businessCategory || l.sector || 'manufacturing',
      businessCategory: l.businessCategory || l.sector || 'manufacturing',
      interestRate: l.interestRate || (l.acieScore?.grade === 'A' ? 13.5 : l.acieScore?.grade === 'B' ? 16.0 : 19.5),
      grade: l.acieScore?.grade || 'B',
      score: l.acieScore?.total || 700,
      fundedAmount: l.fundingStatus?.funded || 0,
      targetAmount: l.fundingStatus?.target || l.loanAmount,
      percentFunded: l.fundingStatus?.percentFunded || Math.round(((l.fundingStatus?.funded || 0) / (l.fundingStatus?.target || l.loanAmount || 1)) * 100),
      fundingPercent: l.fundingStatus?.percentFunded || Math.round(((l.fundingStatus?.funded || 0) / (l.fundingStatus?.target || l.loanAmount || 1)) * 100),
      remainingAmount: Math.max(0, (l.fundingStatus?.target || l.loanAmount) - (l.fundingStatus?.funded || 0)),
