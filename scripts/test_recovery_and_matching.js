...
const Borrower = require('../backend/models/Borrower');
const Lender = require('../backend/models/Lender');
const LoanApplication = require('../backend/models/LoanApplication');
const LoanRepayment = require('../backend/models/LoanRepayment');
const MatchingEngine = require('../backend/services/matchingEngine');
const RecoveryEngine = require('../backend/services/recoveryEngine');
...
async function runTests() {
  ...
  // 1. Test Matching Engine for Vikram (Conservative)
  const vikram = await Lender.findOne({ name: 'Vikram Sethi' });
  const matchesVikram = await MatchingEngine.getMatchesForLender(vikram.lenderId);
  console.log(`[TEST 1] Vikram (Conservative) Matches: ${matchesVikram.length} loan(s). Grades: ${matchesVikram.map(m => m.grade).join(', ')}`);
  assert(matchesVikram.every(m => m.grade === 'A'), 'Conservative lender must only match Grade A');

  // 2. Test Matching Engine for Karan (Aggressive)
  ...
  const prevFunded = priyaLoan.fundingStatus.funded;
